/*
 * Xibalba eBPF Injector: Link Tracepoint
 * 
 * Injects delays at link/unlink syscall exits to expose transaction
 * abort "dirty read" scenarios.
 * 
 * Targets:
 ...
 * - Use-after-free bugs
 * 
 * Based on: docs/design/BTRFS-TRANSACTION-ABORT-RACES.md
...
int trace_link_exit(struct trace_event_raw_sys_exit *ctx) {
    ...
    // - Inode reference count incremented
    // - Directory entry added
    // - Transaction may still be in flight (btrfs)
    // 
    // Delaying HERE allows:
    // - Other transactions to read the new inode ref
    // - If this transaction later aborts, they have STALE DATA!
    // - This is the "dirty read" window
...
int trace_unlink_exit(struct trace_event_raw_sys_exit *ctx) {
    ...
    // Unlink succeeded - reduces reference count
    // If this is a hard link unlink, ref count decremented but inode still exists
    // If this is last link, inode will be freed
    // 
    // Delaying here exposes:
    // - Window where inode is marked for deletion but still referenced
    // - Potential use-after-free if another operation reads it
